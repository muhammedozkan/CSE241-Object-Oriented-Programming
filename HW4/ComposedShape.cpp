...
void ComposedShape::optimalCircRect(Circle *mContainer, Rectangle *sShape)
{
	...
	*this += shp;
	...
	optimalRectRect(&middle, sShape); //ortaya eklemeyi kaldır!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11111111111
...
void ComposedShape::optimalFit()
{
	...
	setObjectCount(getPolygons().size() - 1); //substract main shape object count
...
ostream &operator<<(ostream &outs, ComposedShape &obj)
{
	...
	for (decltype(obj.getPolygons().size()) i = 0; i < obj.getPolygons().size();
		 ++i)
	{
		outs << obj[i];
	}
...
ComposedShape &ComposedShape::operator+=(Polygon ply)
{
	_polys.push_back(ply);

	return *this;
}

//reference shapeelem [] operator overloading
Polygon &ComposedShape::operator[](int index) //lvalue
{
	decltype(_polys.size()) ndx = index;
	if (ndx < _polys.size())
	{
	...
	else
	{
		...
		return getPolygons()[0];
	}
}

//reference shapeelem [] operator overloading
const Polygon &ComposedShape::operator[](int index) const //rvalue
{
	decltype(_polys.size()) ndx = index;
	if (ndx < _polys.size())
	{
...
vector<Polygon> &ComposedShape::getPolygons()
{
	return _polys;
}

void ComposedShape::setPolygons(vector<Polygon> polys)
{
	_polys = polys;
}